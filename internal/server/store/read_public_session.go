...
// PublicSessionSnapshot is the bounded state rendered by a public session page or one
// of its earlier-page requests. ProjectionRevision identifies the committed projection
// so a browser cannot append rows from two rebuilds. It carries no whole-session shape:
// the outline rail and flow ribbon are signed-in surfaces, so reading every message and
// tool call of the session to publish them here served no renderer.
...
// PublicSessionByID loads a published session and one transcript window from a single
// repeatable-read snapshot. before is nil for the trailing window and names the first
// currently rendered ordinal when paging backward. Publication is checked inside the
// same transaction as every rendered row.
func (s *Store) PublicSessionByID(ctx context.Context, publicID string, before *int) (PublicSessionSnapshot, error) {
	snap := PublicSessionSnapshot{Audit: SessionAudit{Fallbacks: []ModelFallback{}}}
