...
Start with 'overview' for fleet-wide usage, or 'list_projects' to see projects ranked by recent activity. 'list_sessions' is the cross-project feed with agent/project/user/machine filters; pass a filter value verbatim from the facet counts the same tool returns. List and overview text names the integer ids later tools take (session_id, project_id, user_id). 'get_session' pages one session's transcript by encoded response bytes: messages, thinking, and tool-call metadata. Follow next_after while has_more is true. An oversized message field carries a preview and an authenticated resource link for the full text. Tool inputs and results are stored by content hash, not inline; fetch a body with 'read_tool_body' using the sha256 from a tool call. 'get_session_raw' returns the lossless bytes the session was ingested from, behind the parsed projection.
