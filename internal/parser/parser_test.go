...
func TestParseGrok(t *testing.T) {
	...
	if u.CostUSD == nil || math.Abs(*u.CostUSD-0.00153) > 1e-12 {
		t.Errorf("usage 0 cost = %s, want 0.00153 reported", grokCostText(u.CostUSD))
	}
	u2 := s.UsageEvent[1]
	if u2.Model != "grok-4.5" || u2.Input != 300 || u2.CacheRead != 600 {
		t.Errorf("usage 1 = %+v", u2)
	}
	if u2.CostUSD == nil || math.Abs(*u2.CostUSD-0.00066) > 1e-12 {
		t.Errorf("usage 1 cost = %s, want 0.00066 reported", grokCostText(u2.CostUSD))
	}
